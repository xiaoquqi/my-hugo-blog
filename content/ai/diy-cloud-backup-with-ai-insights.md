...
HyperFileLens 解决的正是这一层：它的 Agent 本身就是用 Go 和 Kopia 写的，备份、去重、加密这套硬核能力原封不动继承自 Kopia，上面加了一个 Web 控制台统一管理多台机器和多个仓库，还接了一个 AI 引擎 SourceLens——不用再手动 mount 出来翻，直接在快照里搜索、阅读、推理原始文件，回答"这份文件里说了什么""这两个版本差在哪"这类问题。
