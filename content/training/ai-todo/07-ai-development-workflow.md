...
1. **浏览器页面**：你能用眼睛看的那个界面。
2. **WorkBuddy 的执行记录**：它在对话里展示的、自己跑命令 / 起服务的过程和输出。
...
WorkBuddy 帮你启动服务、跑命令时，会在对话里展示它执行的过程和输出。你**不需要看懂每一行**，只要认几个关键信号：

- **成功的信号**：出现类似 `running on http://localhost:xxxx`、`Server started`、`Compiled successfully` 这样的字样，通常代表跑起来了。
- **出问题的信号**：出现 `Error`、`Failed`、`Traceback`，或者 WorkBuddy 明确告诉你「启动失败」「报错了」。

> 💡 一个超实用的经验：出现这些**报错字样，基本就是坏消息**；出现「启动成功」「运行在……」这类字样，基本就是好消息。你现在不必读懂报错内容，只要看到它，就知道「这里要原样发给 AI」。

**③ 功能行为**

页面能打开、也没看到报错，还不等于功能对。你得**真的去用一下**：在输入框打字、点添加、看任务有没有真的出现；点删除、看它是不是真的消失。
...
2. 执行记录：启动服务时 WorkBuddy 有没有提示报错？（看**执行记录**）
...
- **别只看一处**。页面好看不代表功能对，没报错也不代表页面对。**三处都扫一眼**，才踏实。
- 记住「看结果」不需要懂代码。你判断的是**现象**——看到了没、动了没、对不对，这些人人都看得懂。

### 本章小结

看结果 = 做判断题，看三处：浏览器页面、WorkBuddy 的执行记录、功能行为。报错字样是坏消息的信号，功能要亲手点一遍才算数。三处都对，才叫真的做好了。
...
> WorkBuddy 执行记录里或浏览器控制台里那段报错，**一个字一个字原封不动地复制**给 AI。不要自己概括成「它说什么找不到」——报错原文里藏着 AI 定位问题的关键信息，你一转述就丢了。
...
- **看结果**：看三处——页面、WorkBuddy 的执行记录、功能行为。
