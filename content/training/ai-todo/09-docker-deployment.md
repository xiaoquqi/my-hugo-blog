...
这一部分技术名词会多一点，但**别慌**。你要做的事和前面完全一样：把需求讲清楚，让 AI 写配置和脚本，你负责一件件验收。原理我们只讲到「够你判断结果」为止，写 Dockerfile、写 docker-compose、写部署脚本这些又细又碎的活，全部交给 WorkBuddy。
...
现在你的 AI Todo 是怎么跑的？你让 WorkBuddy 帮你把它启动，然后在浏览器访问 `http://localhost` 看到它。但 `localhost` 是「我自己这台电脑」——你把这个地址发给朋友，他打不开，因为那指的是**他**的电脑，不是你的。
...
先装好 Docker（如果还没装，直接让 AI 给你对应系统的安装命令）。然后在项目里打开 WorkBuddy，把这个任务提示词发给它：
...
- 项目里出现一个 `Dockerfile`。
- WorkBuddy 帮你「构建」镜像，执行记录里滚出一大串日志，最后提示成功。
- WorkBuddy 帮你「运行」容器，后端启动。
...
```text
你帮我构建镜像并运行了容器，但在浏览器访问 http://localhost:8000/docs 打不开。
容器运行时的日志是这段（原文粘贴）：
<把日志粘在这里>
...
- WorkBuddy 帮你执行那条启动命令，执行记录里三段日志滚动，分别是三个服务在启动。
